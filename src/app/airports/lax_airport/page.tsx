...
function BurbankAirportPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const arrivalsResponse = await fetch(`/api/flights/lax?query=arrivals`);
                const arrivalsData = await arrivalsResponse.json();
                setArrivals(arrivalsData.data);

                const departuresResponse = await fetch(`/api/flights/lax?query=departures`);
    ...
    const getStatusColor = (status: string) => {
        ...
        if (status.includes("Arrived") || status.includes("Landed")|| status.includes("On Time")){
            return "text-green-600 bg-green-50";
        }
        else if(status.includes("Estimated")){
            return "text-amber-500 bg-orange-50"
        }
    ...
    return (
        <>
            <title>Los Angeles Airport Flights Status | LAX Real-Time Arrivals & Departures</title>

            <meta name="description" content="Check real-time flight status for Los Angeles International Airport (LAX). Get live updates on arrivals, departures, delays, terminals, gates, and airline information." />

            <meta name="keywords" content="Los Angeles airport flight status, LAX flight arrivals, LAX flight departures, LAX live flights, LAX airport status, Los Angeles airport delays, LAX terminals, LAX airline info" />

            <meta name="robots" content="index, follow" />
            <meta name="viewport" content="width=device-width, initial-scale=1.0" />
            <meta property="og:title" content="Los Angeles Airport Flights Status | LAX Real-Time Info" />
            <meta property="og:description" content="Get the latest updates on flight arrivals and departures at Los Angeles International Airport (LAX). Stay informed with accurate flight tracking and terminal info." />
            <meta property="og:type" content="website" />
            <meta property="og:url" content="https://www.yourdomain.com/lax-flights-status" />
            <meta property="og:image" content="https://www.yourdomain.com/images/lax-airport.jpg" />

            <main className="nc-PageHome relative overflow-hidden">
                <BgGlassmorphism />
                <div className="container relative space-y-5 mt-8 mb-24 lg:space-y-8 lg:mb-8">
                    <h1 className="text-3xl font-bold">Los Angeles Airport Flights Status</h1>
