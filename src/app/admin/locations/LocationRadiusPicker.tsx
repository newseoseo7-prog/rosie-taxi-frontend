...
export default function LocationRadiusPicker({ onLocationSaved }: LocationRadiusPickerProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Reset form
            setTitle('');
            setPickupArea({ location: null, radius: 1000 });
            setDropoffArea({ location: null, radius: 1000 });
            setFixedFare('');
            setMinimumFare('');
            setDistanceInfo(null);
    ...
    return (
        <div className="p-6">
            <div className="max-w-4xl mx-auto">
                <h1 className="text-3xl font-bold mb-6 text-neutral-900 dark:text-neutral-100">Set Pickup and Dropoff Areas</h1>
                ...
                {isLoaded ? (
                    <div className="mb-8">
                        ...
                        <div className="mt-4">
                            ...
                            <p className="text-center mt-2 text-neutral-700 dark:text-neutral-300">
                        ...
                        {/* ✅ Distance Info */}
                        {distanceInfo && (
                            <div className="bg-gray-50 dark:bg-neutral-700 rounded-lg p-4 mt-4 border border-gray-200 dark:border-neutral-600">
                                <p className="text-neutral-700 dark:text-neutral-300"><strong>Center Distance:</strong> {distanceInfo.distance.toLocaleString()} meters</p>
                                <p className="text-neutral-700 dark:text-neutral-300"><strong>Minimum Possible Distance:</strong> {distanceInfo.minDistance.toLocaleString()} meters</p>
                                <p className="text-neutral-700 dark:text-neutral-300"><strong>Maximum Possible Distance:</strong> {distanceInfo.maxDistance.toLocaleString()} meters</p>
                            </div>
                        )}
                    </div>
                ) : (
...
function AreaCard({ type, active, area, onClick }: {
    ...
    return (
        <div
            ...
            onClick={onClick}
        >
            <h2 className="text-xl font-semibold mb-2 text-neutral-900 dark:text-neutral-100">{type === 'pickup' ? 'Pickup' : 'Dropoff'} Area</h2>
            {area.location ? (
                <div className="space-y-2">
                    <p className="text-neutral-700 dark:text-neutral-300">Latitude: {area.location.lat.toFixed(6)}</p>
                    <p className="text-neutral-700 dark:text-neutral-300">Longitude: {area.location.lng.toFixed(6)}</p>
                    <p className="text-neutral-700 dark:text-neutral-300">Radius: {area.radius.toLocaleString()} meters</p>
                </div>
            ) : (
...
function FareSettings({ fixedFare, minimumFare, onFixedFareChange, onMinimumFareChange }: {
    ...
    return (
        <div className="bg-white dark:bg-neutral-700 rounded-xl shadow-lg border border-gray-100 dark:border-neutral-600 p-6 mb-8">
            <h2 className="text-xl font-semibold mb-4 text-neutral-900 dark:text-neutral-100">Fare Settings</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium mb-1 text-neutral-700 dark:text-neutral-300">Fixed Fare (optional)</label>
                    <input
                        type="number"
                        value={fixedFare}
                        onChange={(e) => onFixedFareChange(e.target.value)}
                        placeholder="Enter fixed fare amount"
                        className="w-full px-3 py-2 border border-gray-300 dark:border-neutral-600 rounded-md bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100"
                        min="0"
                        step="0.01"
                    />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1 text-neutral-700 dark:text-neutral-300">Minimum Fare</label>
                    <input
                        type="number"
                        value={minimumFare}
