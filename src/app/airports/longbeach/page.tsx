...
function SloPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const arrivalsResponse = await fetch(`/api/flights/longbeach?query=arrivals`);
        const arrivalsData = await arrivalsResponse.json();
        setArrivals(arrivalsData.data);

        const departuresResponse = await fetch(`/api/flights/longbeach?query=departures`);
  ...
  return (
      <>
        ...
        <title>Long Beach Airport Flight Status | LGB Arrivals & Departures</title>

        <meta name="description" content="View real-time flight status updates for Long Beach Airport (LGB). Check live arrivals, departures, delays, airline info, and terminal updates for Long Beach, CA." />

        <meta name="keywords" content="Long Beach Airport flight status, LGB flight arrivals, LGB departures, Long Beach live flights, LGB airport delays, LGB terminal info, Long Beach airline schedules" />

        <meta name="robots" content="index, follow" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <meta property="og:title" content="Long Beach Airport Flight Status | Real-Time LGB Arrivals & Departures" />
        <meta property="og:description" content="Track live flight arrivals and departures at Long Beach Airport (LGB). Stay informed about delays, gates, and airline schedules in real-time." />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://www.yourdomain.com/lgb-flight-status" />
        <meta property="og:image" content="https://www.yourdomain.com/images/lgb-airport.jpg" />

        <main className="nc-PageHome relative overflow-hidden">
          <BgGlassmorphism/>
          <div className="container relative space-y-5 mt-8 mb-24 lg:space-y-8 lg:mb-8">
            <h1 className="text-3xl font-bold">Long Beach Airport Flight Status</h1>
