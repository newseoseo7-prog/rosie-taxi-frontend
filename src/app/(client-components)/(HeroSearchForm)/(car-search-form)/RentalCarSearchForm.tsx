...
const RentalCarSearchForm: FC<RentalCarSearchFormProps> = ({}) => {
  ...
  const handlePickupLocationSelect = (place: google.maps.places.PlaceResult) => {
    setPickupLocation(place);
    if (dropOffLocationType === "same") {
      setDropoffLocation(place);
    }
  };

  const handleDropoffLocationSelect = (place: google.maps.places.PlaceResult) => {
    setDropoffLocation(place);
  };
    ...
    return (
      <div className=" py-5 [ nc-hero-field-padding ] flex items-center flex-wrap flex-row border-b border-neutral-100 dark:border-neutral-700">
        <div
          ...
          onClick={(e) => {
            setDropOffLocationType("different");
